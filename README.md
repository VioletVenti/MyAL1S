...
- **可控写操作** — 交作业 / 选课 / 发帖等副作用操作，受用户配置的权限矩阵约束（默认「需确认」）。**P2 已落地交作业**：在作业行点「交作业」直接提交（UI 直交，即点即传），或在对话里附文件让助手发起——助手创建审批后**对话框输入框上方弹出横向审批条**，点确认才真正执行；两种入口共享同一道权限闸，写操作的执行确定性地直达教学网、**绝不经 LLM**。选课 / 发帖随 P3 接入。
...
- **登录即初始化 + 重启留存** — 输一次 OTP 后后台预热全部数据源；后端快照缓存让**重启后端/未登录时仍显示上次内容**（标「离线缓存」角标），前端 localStorage 让刷新瞬间显示。**未连接时仪表盘统一显示「未连接，请登录」**（一次连接检查，而非每个面板各自转圈）。
...
- **对话升级** — 模型选择（Claude / Kimi / OpenAI 兼容等）、每轮可折叠的「工具调用 / 思考」、新对话、**对话框顶部常驻的历史会话列表**（一键回到旧对话）。写操作（交作业）需要确认时，**输入框上方弹出横向审批条**，点确认/拒绝即在对话里完成，不必跳到别处。
