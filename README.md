...
**状态：** ✅ **P0 竖切 + P1 数据面板 + P2 写操作（交作业）已打通**。P1 = 周历 + 待办 + 新到通知 + 课程通知/材料/回放 + 星标/自定义待办 + 对话升级。**P2 = 可控写操作**：交作业（UI 直交 + agent 两阶段审批），受权限矩阵约束（默认「需确认」），执行确定性、不过 LLM。后续 P3–P4（外部论坛/选课/发帖、凭据加密、流式 chat）见 roadmap。
