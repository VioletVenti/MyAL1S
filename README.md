...
**状态：** ✅ **P0 竖切 + P1 数据面板 + P2 写操作 + P3 树洞 已打通**。**P3 = 北大树洞**：轻量通知面板（关注更新 + 未读数）+ 9 个树洞 MCP 工具（列表/详情/楼层/搜索/通知/未读/关注/我的/历史）+ 发帖/回复（经权限矩阵，默认「需确认」，可选 auto 自主）+ agent 自然语言搜树洞。后续 P4（凭据加密、流式 chat、选课）见 roadmap。
...
- **可控写操作** — 交作业 / 发帖等副作用操作，受用户配置的权限矩阵约束（默认「需确认」，可选「自动」/「禁止」）。**P2 交作业**：作业行点「交作业」直接提交，或对话里附文件让助手发起审批。**P3 树洞发帖/回复**：助手发起后对话框上方弹审批横条。均可设为「自动」让 agent 直接执行（文件无关写）。两种入口共享同一道权限闸，写操作的执行确定性地直达、**绝不经 LLM**。

**P1 数据面板** 在聚合层之上加了「个人化日程」视角：

- **主界面（极简）** — 周历 + 课表（干净课名，点击日期展开当天星标/待办）+ 待办 + 新到通知 + 对话侧栏。顶部「主界面 / 目录」切换。
- **目录视图** — 左侧栏点选模块、右侧一次只看一个（作业 / 课程通知 / 课程材料 / 课程回放 / 成绩 / **北大树洞** + 三个「待接入」面板），列表分页。
- **登录即初始化 + 重启留存** — 输一次 OTP 后后台预热全部数据源；后端快照缓存让**重启后端/未登录时仍显示上次内容**（标「离线缓存」角标），前端 localStorage 让刷新瞬间显示。面板始终渲染（快照优先），不会先被「正在连接」盖住。
...
- **对话升级** — 模型选择（Claude / Kimi / OpenAI 兼容等）、每轮可折叠的「工具调用 / 思考」、新对话、**对话框顶部常驻的历史会话列表**（一键回到旧对话）。写操作（交作业 / 树洞发帖）需要确认时，**输入框上方弹出横向审批条**，点确认/拒绝即在对话里完成，不必跳到别处。

**P3 北大树洞** 已接入：

- **轻量通知** — 面板显示树洞未读数 + 关注帖子更新（不爬全量），刷新即显示快照。
- **搜索 + agent** — 对话里说「帮我搜树洞里关于绩点的讨论」，助手调 `treehole_search` 关键词搜索；也可读通知/楼层/详情/我的/历史/关注。
- **发帖 / 回复** — 助手发起后走权限矩阵（默认需确认，可设「自动」让 agent 直接发）。
